...
def main():
    ...
    # クエリ画像を読み込んで特徴量計算
    query_img = cv2.imread('../data/images/corridor/running4/color/000160.jpg', 0)
    ...
    # マップ画像を読み込んで特徴量計算
    map_img = cv2.imread('../data/images/corridor/running4/color/000161.jpg', 0)
    ...
    best = sorted(best, key=lambda x: x[0].distance)
    img1_pt = [list(map(int, kp_query[m[0].queryIdx].pt)) for m in best]
    img2_pt = [list(map(int, kp_map[m[0].trainIdx].pt)) for m in best]
    print (img1_pt[0], img2_pt[0])
    print (img1_pt[1], img2_pt[1])
    print (img1_pt[2], img2_pt[2])
    # 精度が高かったもののうちスコアが高いものから指定個取り出す
    good = sorted(good, key=lambda x: x[0].distance)
    print("valid point number: ", len(good))  # これがあまりに多すぎたり少なすぎたりする場合はパラメータを変える
    point_num = 3  # 上位何個の点をマッチングに使うか（重要パラメータ）
